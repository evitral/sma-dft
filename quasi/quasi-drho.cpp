...
/* Load/save parameters */
  ...
  std::string strLoad = "/oasis/scratch/comet/evitral/temp_project/quasi_flat/qsi-drho-nw8-nu";
  ...
  std::string strBox = "/oasis/scratch/comet/evitral/temp_project/quasi_flat/qsi-drho-nw8-nu";
...
/* Balance of Linear Momentum parameters */
  ...
  double Amp = 1.328; //1.328; 
  double rho_0 = 0.1; // 0.01 .. 0.05 worked with 0.5
  double kp = 5.0; // 0.5 for nw= 8
  double rho_m = (kp*pow(Amp,1)+rho_0)/2;

  double mb = 0.1; // mobility
 ...
 *******************************************/
  ...
  for ( i_local = 0; i_local < local_n0; i_local++ )
  {
    ...
    for ( k = 0; k < Nz; k++ )
    {
      ...
      aLin[index] = ep - opSH;
      C1[index] = (2.0-mb*dt*rho_m*aLin[index]);
      C2[index] = (1.5-mb*dt*rho_m*aLin[index]);
  ...
  *******************************************/
  ...
  while (L1 > limL1)
  {
	...
	*exp(-1.57*1.57*mq2c[index]/2);	     	       
	...
	if (nLoop > 53){
	Nl_local[index] +=
	  p_local[index]*dRho_local[index]/(2*pow(rho_local[index],2));	  
