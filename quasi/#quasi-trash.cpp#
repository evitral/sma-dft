 ...
 *                                                        *
 *    Note: gradients of rho are neglected                *
 *                                                        *
...
/* Load/save parameters */
  ...
  std::string strLoad = "/oasis/scratch/comet/evitral/temp_project/quasi_fc/gradrho_off/qsi2-fc140-nw8-nu";
  ...
  std::string strBox = "/oasis/scratch/comet/evitral/temp_project/quasi_fc/gradrho_off/qsi2-fc140-nw8-nu";
	...
	*exp(-1.57*1.57*mq2c[index]*8);	   // /2  	       
	...
	Nl_local[index] +=
	  ...
	  sqrt(pow(rhoDx_local[index],2)+pow(rhoDy_local[index],2)
	       +pow(rhoDz_local[index],2))/pow(rho_s,2);
	...
	// Save divv for mid cross section
	...
	for( i_local = 0; i_local < local_n0 ; i_local++ ){
	  index  = (i_local*Ny +j)*Nz + k;
	  index2 = i_local*Nz + k;
	  psiSlice_local[index2] = p_local[index];
	}}
