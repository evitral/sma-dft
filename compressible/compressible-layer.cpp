 ...
 *    i)  Vitral et al, PRE 100(3) 032805                 *
 *    ii) Vitral et al, PRF (5) 093501                    *
 *    iii) Vitral et al, Soft Matter, 2021, 17            *
 *                                                        *
 ...
 *                                                        *
 *     Layer: control parameter boudary layer close to    *
 *     channel's borders                                  *
 *                                                        *
 **********************************************************/
...
mpicxx -O2 -o code compressible.cpp -lfftw3_mpi -lfftw3 -lm -std=c++11
...
/* Load/save parameters */
  ...
  std::ofstream psiMid_output, psiYZ_output, surf_output, velS_output, 
    ...
    energyMid_output, surface_output, psibottom_output;
...
********************************************/
    ...
    for ( i_local = 0; i_local < local_n0; i_local++ ) 
    {
	  ...
	  if (k < Nw*2) {
	    ...
	    double stretch = 0*(1-(double)k/(Nw*2))+1;	    
	    ys = (j+64 - mid)/stretch;
	  } else {
	    ys = j+64 - mid;
	  }
...
/* B. Initial condition - Read profile data */
  ...
  if ( load == 1 )
  {
    ...
    strPsi += std::to_string(rank);
    strPsi += ".dat";
    strLoad2 = strLoad + strPsi;
    strPsi  = strSave + strPsi;	
    
    std::ifstream psidata(strLoad2.c_str());
...
*/	
  ...
  {	
    ...
    // Psi Bottom (XY) output
    std::ofstream psibottom_output(strBox+"psiBottom.dat");
    assert(psibottom_output.is_open());
    psibottom_output.close();		
  ...
  *******************************************/
  ...
  while (L1 > limL1)
  {
	...
	*exp(-1.57*1.57*mq2c[index]*2);	   // /2 *8  	       
	...
	saveMid(Nx, Ny, Nz, local_n0, rank, alloc_slice, psiSlice_local, psiSlice,
		rho_local, rhoMid_output, strBox, "rhoMid.dat");
	...
	if (rank == 0 )
	{
	  psibottom_output.open(strBox+"psiBottom.dat",std::ios_base::app);					
