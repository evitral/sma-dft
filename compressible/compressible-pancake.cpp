 ...
 *    ii) Vitral et al, PRF ?                             *
 *    iii) Vitral et al, in preparation                   *
 *                                                        *
...
void saveMidXY(int Nx, int Ny, int Nz, int local_n0, long long rank, double alloc_surf, 
	     std::vector<double> psiSliceXY_local, std::vector<double> psiSliceXY,
	     std::vector<double> target_local, std::ofstream& target_output,
	     std::string strBox, std::string target_file)
{
  int i, j, k, i_local, index, index2;
  ...
  for( i_local = 0; i_local < local_n0 ; i_local++ ){
    index  = (i_local*Ny +j)*Nz + k;
    index2 = i_local*Ny + j;
    psiSliceXY_local[index2] = target_local[index];
  }}

  MPI::COMM_WORLD.Gather(psiSliceXY_local.data(),alloc_surf,MPI::DOUBLE,
			 psiSliceXY.data(),alloc_surf, MPI::DOUBLE,0);
  ...
  if (rank == 0 )
  {
    ...
    for ( j = 0; j < Ny; j++ ) {
      ...
      target_output << psiSliceXY[index] << "\n";
    }}
...
/* Load/save parameters */
  ...
  std::string strLoadRho = strLoad + argv[1] + std::string("-e0d") + argv[2] 
    + std::string("/save/");
  ...
  std::ofstream psiMid_output, surf_output, velS_output, 
    ...
    psiXY_output, psiYZ_output, rhoXZ_output, rhoXY_output, rhoYZ_output, 
    ampXY_output, ampYZ_output;
  ...
  const int stepSave = 40; // 10
  const int divvSwitch = 50; // 100 or 200
...
/* B. Initial condition - Read profile data */
  ...
  if ( load == 1 )
  {
    ...
    strRho += std::to_string(rank);
    strRho += ".dat";
    strLoad = strLoad + strRho;
    strRho  = strBox + strRho;	
    
    std::ifstream rhodata(strLoadRho.c_str());
...
*/	
  ...
  {	
    ...
    std::ofstream psiXY_output(strBox+"psiXY.dat");
    assert(psiXY_output.is_open());
    psiXY_output.close();
    ...
    std::ofstream rhoXZ_output(strBox+"rhoXZ.dat");
    assert(rhoXZ_output.is_open());
    rhoXZ_output.close();

    std::ofstream rhoXY_output(strBox+"rhoXY.dat");
    assert(rhoXY_output.is_open());
    rhoXY_output.close();

    std::ofstream rhoYZ_output(strBox+"rhoYZ.dat");
    assert(rhoYZ_output.is_open());
    rhoYZ_output.close();
    ...
    std::ofstream ampXY_output(strBox+"ampXY.dat");
    assert(ampXY_output.is_open());
    ampXY_output.close();

    std::ofstream ampYZ_output(strBox+"ampYZ.dat");
    assert(ampYZ_output.is_open());
    ampYZ_output.close();
 ...
 *******************************************/
 ...
 // for(int tst=0;tst < 2;tst++) 
  while (L1 > limL1)
  {
    ...
    // std::fill(ampDx_local.begin(),ampDx_local.end(),0);
    // std::fill(ampDy_local.begin(),ampDy_local.end(),0);
    // std::fill(ampDz_local.begin(),ampDz_local.end(),0);
    ...
    for( k = 0; k < Nz; k++ ){
      ...
      //psi_back3[index] = ampq_local[index2];
    }}
	...
	//ampDx_local[index] = -Vsx[i_local]*psi_front3[index2]; 
      }}
    }
    ...
    for ( k = 0; k < Nz; k++ )
    {
      ...
      //ampDx_local[index] = -Vsx[i_local]*ampq_local[index2];
    }}}
    ...
    for ( k = 0; k < Nz; k++ )
    {
      ...
      //ampDy_local[index] = -Vsy[j]*ampq_local[index2];
    }}}	
    ...
    for ( k = 0; k < Nz-1; k++ )
    {
      ...
      //ampDz_local[index] = -Vsz[k]*ampq_local[index2];	   
    }}}	
    ...
    // Move grad A to real space

    // trans_local = ampDx_local;
    // fftw_execute(iPlanSTx);
    // ampDx_local = trans_local;

    // trans_local = ampDy_local;
    // fftw_execute(iPlanSTy);
    // ampDy_local = trans_local;

    // trans_local = ampDz_local;
    // fftw_execute(iPlanSTz);
    // ampDz_local = trans_local;
	...
	*exp(-1.57*1.57*mq2c[index]*2);	   // /2 *8  	       
    ...
    //   if (i+j+k != 0){
    //   	trans_local[index] = 
    //   	  ((Vqx[i_local]*Sx+Vqy[j]*Sy+Vqz[k]*Sz)/mq2c[index]);
    //   } else {
    //   	trans_local[index] = 0;
    //   }
	...
	psi_back2[index] = 0;
      ...
      index2 = (i_local*Ny + j) * Nz + k;
      psi_back3[index] = fgrad_local[index2]; //fgrad_local[index2];
    }}
    ...
    {
      std::fill(psi_back.begin(),psi_back.end(),0);
      std::fill(psi_back2.begin(),psi_back2.end(),0);
    }
	...
	// 	 +pow(rhoDz_local[index],2))/pow(rho_local[index],2);   // or rho_s i
	//  f low rho_a
	// }
      }}}
	...
	if( i == Nx/2)
	{
	  ...
	  psiYZ_output.open(strBox+"psiYZ.dat",std::ios_base::app);
	  rhoYZ_output.open(strBox+"rhoYZ.dat",std::ios_base::app);
	  //ampYZ_output.open(strBox+"ampYZ.dat",std::ios_base::app);
	  assert(psiYZ_output.is_open());
	  assert(rhoYZ_output.is_open());
	  //assert(ampYZ_output.is_open());
	  ...
	  for ( k = 0; k < Nz; k++ )
	  {
	    ...
	    psiYZ_output << psi_local[index] << "\n";
	    rhoYZ_output << rho_local[index] << "\n";
	    //ampYZ_output << amp_local[index] << "\n";
	...
	saveMid(Nx, Ny, Nz, local_n0, rank, alloc_slice, psiSlice_local, psiSlice,
		psi_local, psiMid_output, strBox, "psiMid.dat");
	...
	saveMid(Nx, Ny, Nz, local_n0, rank, alloc_slice, psiSlice_local, psiSlice,
		rho_local, rhoXZ_output, strBox, "rhoXZ.dat");

	saveMidXY(Nx, Ny, Nz, local_n0, rank, alloc_surf, psiSliceXY_local, 
		  psiSliceXY, rho_local, rhoXY_output, strBox, "rhoXY.dat");
	...
	// saveMidXY(Nx, Ny, Nz, local_n0, rank, alloc_surf, psiSliceXY_local, 
	// 	  psiSliceXY, amp_local, ampXY_output, strBox, "ampXY.dat");
