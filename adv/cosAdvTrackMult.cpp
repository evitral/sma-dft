...
/* Load/save parameters */
	...
	std::string strLoad = "np1024Init800Move0d8/";
	
	std::ofstream psiMid_output, surf_output, velS_output, curvH_output, curvK_output, sx_output, sy_output, sz_output;
...
/* Constants and variables for morphologies (Nx = Ny = Nz) */
	...
	const double aE = 270; // 270 (FC) // 80 // 312 // 432 // 248 // 810
	const double bE = 270; // 270 (FC) // 86 // 376 // 520 // 248 // 810
...
/* Phase Field parameters */
	...
	const double ep    = -0.7; // -0.7 CHANGED !!!!!!!!!!!!!!
	const double q0    =  1.0;
...
/* Local data containers */
	...
	std::vector<double> C3(alloc_local);
...
/* Local data containers (advection) */
	...
	std::vector<double> psiDxx_temp(Nslice);
	std::vector<double> psiDxx_front(Nslice);
	std::vector<double> psiDxx_back(Nslice);

	std::vector<double> psiDyy_temp(Nslice);
	std::vector<double> psiDyy_front(Nslice);
	std::vector<double> psiDyy_back(Nslice);

	std::vector<double> psiDzz_temp(Nslice);
	std::vector<double> psiDzz_front(Nslice);
	std::vector<double> psiDzz_back(Nslice);
 ...
 *******************************************/
	...
	iPlanSp2 = fftw_mpi_plan_r2r_3d(Nx,Ny,Nz,
				  Sp2_local.data(),Sp2_local.data(),MPI::COMM_WORLD,
		   	      FFTW_REDFT01,FFTW_REDFT01,FFTW_REDFT01,
		          FFTW_MEASURE);
...
/* Output IC to file and create L1 output */
	...
	/** Create psiMid output **/
		...
		std::ofstream sx_output("vx.dat");
		assert(sx_output.is_open());
		sx_output.close();

		std::ofstream sy_output("vy.dat");
		assert(sy_output.is_open());
		sy_output.close();

		std::ofstream sz_output("vz.dat");
 ...
 *******************************************/
	 ...
	 for ( k = 0; k < Nz; k++ )
	 {
		 ...
		 //C3[index] = pow(Vqx[i_local],4)+pow(Vqy[j],4)+pow(Vqz[k],4)
		 //  +2*pow(Vqx[i_local],2)*pow(Vqy[j],2)
		 //  +2*pow(Vqx[i_local],2)*pow(Vqz[k],2)
		 //  +2*pow(Vqy[j],2)*pow(Vqz[k],2)-q02*mq2;
 ...
 */		
 ...
 /* Compute new psi in Fourier Space (CN/AB scheme) */
		 ...
		 for ( k = 0; k < Nz; k++ )
		 {
			 ...
			 dTpsi_local[index] = scale*(aLin[index]*psiq_local[index]
										 +Nq_local[index]);
			 ...
			 Sp2_local[index] = scale*C3[index]*psiq_local[index];
 ...
 /* Obtain new psi in real space */
		 ...
		 fftw_execute(iPlanSp2);		 

		 /** Compute gradient of psi **/

		 // Distribute data for FD in the X direction
		 ...
		 if (rank % 2 == 0){ // rank is even
			 ...
			 for( k = 0; k < Nz ; k++ ){
				 ...
				 psi_temp[index2] = psi_local[index];
				 psiDxx_temp[index2] = psiDxx_local[index];
				 psiDyy_temp[index2] = psiDyy_local[index];
				 psiDzz_temp[index2] = psiDzz_local[index];		 
			 }}

			 MPI::COMM_WORLD.Send(psi_temp.data(),Nslice,MPI::DOUBLE,
								  rank+1,0);

			 MPI::COMM_WORLD.Send(psiDxx_temp.data(),Nslice,MPI::DOUBLE,
								  rank+1,1);

			 MPI::COMM_WORLD.Send(psiDyy_temp.data(),Nslice,MPI::DOUBLE,
								  rank+1,2);

			 MPI::COMM_WORLD.Send(psiDzz_temp.data(),Nslice,MPI::DOUBLE,
								  rank+1,3);
			 ...
			 MPI::COMM_WORLD.Recv(psi_back.data(),Nslice,MPI::DOUBLE,
								  rank-1,0);

			 MPI::COMM_WORLD.Recv(psiDxx_back.data(),Nslice,MPI::DOUBLE,
								  rank-1,1);

			 MPI::COMM_WORLD.Recv(psiDyy_back.data(),Nslice,MPI::DOUBLE,
								  rank-1,2);

			 MPI::COMM_WORLD.Recv(psiDzz_back.data(),Nslice,MPI::DOUBLE,
								  rank-1,3);				 
		 ...
		 if (rank % 2 == 0){   // even
			 ...
			 MPI::COMM_WORLD.Recv(psi_front.data(),Nslice,MPI::DOUBLE,
								  rank+1,0);

			 MPI::COMM_WORLD.Recv(psiDxx_front.data(),Nslice,MPI::DOUBLE,
								  rank+1,1);

			 MPI::COMM_WORLD.Recv(psiDyy_front.data(),Nslice,MPI::DOUBLE,
								  rank+1,2);

			 MPI::COMM_WORLD.Recv(psiDzz_front.data(),Nslice,MPI::DOUBLE,
								  rank+1,3);
				 
				 
			 if (rank != 0){   // note the first core (rank 0)
				 i_local = 0;
				 for( j = 0; j < Ny ; j++ ){
				 for( k = 0; k < Nz ; k++ ){
		 ...
		 else if(rank != (size-1)) { // odd, not the last core
		 ...
		 // Finite differences for Neumann BCs
	 	 ...
	 	 for ( k = 0; k < Nz; k++ ) 
		 {
			 ...
			 index3 = ((i_local-1)*Ny+j)*Nz+k;
			 index4 = ((i_local+1)*Ny+j)*Nz+k;
			 index5 = j*Nz+k;

			 if (i_local == 0){
			 
				 if ( rank == 0){
					 psiGradx_local[index] = 0;
				 } else {
					 psiGradx_local[index] =
						 (psi_local[index4] - psi_back[index5])/tdx;
				 }
				 ...
				 if ( rank == size-1 ){
					 ...
					 psiGradx_local[index] =
						 (psi_front[index5] - psi_local[index3])/tdx;
				 }

			 } else {
				 psiGradx_local[index] =
					 (psi_local[index4] - psi_local[index3])/tdx;
			 }

			 switch(j){
			 case 0:
			 case Ny-1:
				 psiGrady_local[index] = 0;
				 break;
			 default:
				 psiGrady_local[index] =
					 (psi_local[index2] - psi_local[index1])/tdy;
			 }

			 switch(k){
			 case 0:
			 case Nz-1:
				 psiGradz_local[index] = 0;
				 break;
			 default:
				 psiGradz_local[index] =
					 (psi_local[index+1] - psi_local[index-1])/tdz;
			 }
	   	 ...
	   	 for ( k = 0; k < Nz; k++ ) 
		 {
			 ...
			 if (i_local == 0){
				 ...
				 if ( rank == 0){
					 ...
					 psiDxxx =
						 (psiDxx_local[index4] - psiDxx_back[index5])/tdx;
					 psiDyyx =
						 (psiDyy_local[index4] - psiDyy_back[index5])/tdx;
					 psiDzzx =
						 (psiDzz_local[index4] - psiDzz_back[index5])/tdx;
				 }
				 ...
				 if ( rank == size-1 ){
					 ...
					 psiDxxx =
						 (psiDxx_front[index5] - psiDxx_local[index3])/tdx;
					 psiDyyx =
						 (psiDyy_front[index5] - psiDyy_local[index3])/tdx;
					 psiDzzx =
						 (psiDzz_front[index5] - psiDzz_local[index3])/tdx;
				 }

			 } else {
				 psiDxxx =
					 (psiDxx_local[index4] - psiDxx_local[index3])/tdx;
				 psiDyyx =
					 (psiDyy_local[index4] - psiDyy_local[index3])/tdx;
				 psiDzzx =
					 (psiDzz_local[index4] - psiDzz_local[index3])/tdx;
			 }
	 
			 switch(j){
			 case 0:
				 psiDxy = (psiGradx_local[index2]-psiGradx_local[index])/dy;
				 psiDxxy = (psiDxx_local[index2]-psiDxx_local[index])/dy;
				 psiDyyy = (psiDyy_local[index2]-psiDyy_local[index])/dy; 
				 psiDzzy = (psiDzz_local[index2]-psiDzz_local[index])/dy;
				 break;
			 case Ny-1:
				 psiDxy = (psiGradx_local[index]-psiGradx_local[index1])/dy;
				 psiDxxy = (psiDxx_local[index]-psiDxx_local[index1])/dy;
				 psiDyyy = (psiDyy_local[index]-psiDyy_local[index1])/dy;
				 psiDzzy = (psiDzz_local[index]-psiDzz_local[index1])/dy;
				 break;
			 default:
				 psiDxy = (psiGradx_local[index2]-psiGradx_local[index1])/tdy;
				 psiDxxy = (psiDxx_local[index2]-psiDxx_local[index1])/tdy;
				 psiDyyy = (psiDyy_local[index2]-psiDyy_local[index1])/tdy;
				 psiDzzy = (psiDzz_local[index2]-psiDzz_local[index1])/tdy;
			 }
				
			 switch(k){
			 case 0:
				 psiDxz = (psiGradx_local[index+1]-psiGradx_local[index])/dz;
				 psiDyz = (psiGrady_local[index+1]-psiGrady_local[index])/dz;
				 psiDxxz = (psiDxx_local[index+1]-psiDxx_local[index])/dz;
				 psiDyyz = (psiDyy_local[index+1]-psiDyy_local[index])/dz;
				 psiDzzz = (psiDzz_local[index+1]-psiDzz_local[index])/dz;
				 break;
			 case Nz-1:	
				 psiDxz = (psiGradx_local[index]-psiGradx_local[index-1])/dz;
				 psiDyz = (psiGrady_local[index]-psiGrady_local[index-1])/dz;
				 psiDxxz = (psiDxx_local[index]-psiDxx_local[index-1])/dz;
				 psiDyyz = (psiDyy_local[index]-psiDyy_local[index-1])/dz;
				 psiDzzz = (psiDzz_local[index]-psiDzz_local[index-1])/dz;
				 break;
			 default:
				 psiDxz = (psiGradx_local[index+1]-psiGradx_local[index-1])/tdz;
				 psiDyz = (psiGrady_local[index+1]-psiGrady_local[index-1])/tdz;
				 psiDxxz = (psiDxx_local[index+1]-psiDxx_local[index-1])/tdz;
				 psiDyyz = (psiDyy_local[index+1]-psiDyy_local[index-1])/tdz;
				 psiDzzz = (psiDzz_local[index+1]-psiDzz_local[index-1])/tdz;
			 }


			 Sx_local[index] =
			   alpha*(
				  //-(psiDxxx+psiDyyx+psiDzzx)*(psiDxx_local[index]
				  //		      + psiDyy_local[index]+psiDzz_local[index]+q02*psi_local[index])
				  //+
				  psiGradx_local[index]*Sp2_local[index]);

			 Sy_local[index] =
			   alpha*(
				  //-(psiDxxy+psiDyyy+psiDzzy)*(psiDxx_local[index]
				  //			      + psiDyy_local[index]+psiDzz_local[index]+q02*psi_local[index])
				  //+ 
				  psiGrady_local[index]*Sp2_local[index]);

			 Sz_local[index] =
			   alpha*(
				  //-(psiDxxz+psiDyyz+psiDzzz)*(psiDxx_local[index]
				  //			      + psiDyy_local[index]+psiDzz_local[index]+q02*psi_local[index])
				  //+ 
				  psiGradz_local[index]*Sp2_local[index]);			 			


/*
			 Sx_local[index] =
				 alpha*(psiDxx_local[index]*(psiDxxx+psiDyyx+psiDzzx
											 + q02*psiGradx_local[index])
						+ psiDxy*(psiDxxy+psiDyyy+psiDzzy
								  + q02*psiGrady_local[index])
						+ psiDxz*(psiDxxz+psiDyyz+psiDzzz
								  + q02*psiGradz_local[index])
						+ psiGradx_local[index]*Sp2_local[index]);

			 Sy_local[index] =
				 alpha*(psiDxy*(psiDxxx+psiDyyx+psiDzzx
								+ q02*psiGradx_local[index])
						+ psiDyy_local[index]*(psiDxxy+psiDyyy+psiDzzy
											   + q02*psiGrady_local[index])
						+ psiDyz*(psiDxxz+psiDyyz+psiDzzz
								  + q02*psiGradz_local[index])
						+ psiGrady_local[index]*Sp2_local[index]);

			 Sz_local[index] =
				 alpha*(psiDxz*(psiDxxx+psiDyyx+psiDzzx
								+ q02*psiGradx_local[index])
						+ psiDyz*(psiDxxy+psiDyyy+psiDzzy
								  + q02*psiGrady_local[index])
						+ psiDzz_local[index]*(psiDxxz+psiDyyz+psiDzzz
											   + q02*psiGradz_local[index])
						+ psiGradz_local[index]*Sp2_local[index]);
*/
		 }}}
	...
	/** Appropriate way to compute curvatures for a TFCD **/
		...
		psi_output.close();
				...
				if (rank == 0 )
				{
					...
					surf_output.open("surfPsi.dat",std::ios_base::app);
					velS_output.open("velSurf.dat",std::ios_base::app);
					curvH_output.open("curvH.dat",std::ios_base::app);
					curvK_output.open("curvK.dat",std::ios_base::app);
...
*/		
	/** Inform date and time after each save psi **/
		...
		for( i_local = 0; i_local < local_n0 ; i_local++ ){
			index  = (i_local*Ny +j)*Nz + k;
			index2 = i_local*Nz + k;
			psiSlice_local[index2] = Sx_local[index];
		}}
		...
		if (rank == 0 )
		  {
		    sx_output.open("vx.dat",std::ios_base::app);
		...
		for( i_local = 0; i_local < local_n0 ; i_local++ ){
			index  = (i_local*Ny +j)*Nz + k;
			index2 = i_local*Nz + k;
			psiSlice_local[index2] = Sy_local[index];
		}}
		...
		if (rank == 0 )
		  {
		    sy_output.open("vy.dat",std::ios_base::app);
		...
		for( i_local = 0; i_local < local_n0 ; i_local++ ){
			index  = (i_local*Ny +j)*Nz + k;
			index2 = i_local*Nz + k;
			psiSlice_local[index2] = Sz_local[index];
		}}
		...
		if (rank == 0 )
		  {
		    sz_output.open("vz.dat",std::ios_base::app);
